...
class SFT_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.linear(x1.reshape(x1.shape[0], 1, -1))  # [batch, 1, 32*2*2] -> [batch, 1, 64]
        x2 = self.linear(x2.reshape(x2.shape[0], 1, -1))
        x3 = self.linear(x3.reshape(x3.shape[0], 1, -1))
        x4 = self.linear(x4.reshape(x4.shape[0], 1, -1))
        x5 = self.linear(x5.reshape(x5.shape[0], 1, -1))
        x6 = self.linear(x6.reshape(x6.shape[0], 1, -1))
        x7 = self.linear(x7.reshape(x7.shape[0], 1, -1))
        x8 = self.linear(x8.reshape(x8.shape[0], 1, -1))
        x9 = self.linear(x9.reshape(x9.shape[0], 1, -1))
        x10 = self.linear(x10.reshape(x10.shape[0], 1, -1))
        x11 = self.linear(x11.reshape(x11.shape[0], 1, -1))
        x12 = self.linear(x12.reshape(x12.shape[0], 1, -1))
        x13 = self.linear(x13.reshape(x13.shape[0], 1, -1))
        x14 = self.linear(x14.reshape(x14.shape[0], 1, -1))
        x15 = self.linear(x15.reshape(x15.shape[0], 1, -1))
        x16 = self.linear(x16.reshape(x16.shape[0], 1, -1))
